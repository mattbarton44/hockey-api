<template>
  <div class="w-full">
    <div class="mb-4 font-semibold flex justify-between">
      ...
      <div>
        <button class="text-xs bg-green-400 px-2 py-1 rounded-sm hover:bg-green-600 hover:text-white" @click="navigateTo(`/clubs/new`)">
          Create Club
        </button>
      </div>
    </div>
      ...
      <table class="min-w-full divide-y divide-gray-300">
        ...
        <tbody>
          <tr v-for="row in data" class="hover:bg-slate-100 cursor-pointer" @click="nav(row.id)">
            ...
            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500"><div>{{ row.venueName }}</div><div>{{ row.venueAddress }}</div></td>
...
let data = ref(null);
axios.get('http://localhost/api/clubs')
.then((response) => {
  data.value = response.data.data;
})
.catch((e) => console.log(e))

const nav = (id) => navigateTo('/clubs/'+id)
