<template>
  <div class="bg-gray-100 p-8">
    <div class="grid xl:grid-cols-3 lg:grid-cols-2 grid-cols-1 gap-4" v-if="data">
      ...
      <hockey-result :data="{ ...data.games.find((g) => g.status === 'COMPLETED'), competition: data.competition }" >
        <div class="text-base font-semibold py-6 px-6 text-blue-500">Hockey Result</div>
      </hockey-result>
      <hockey-result :data="{ ...data.games.find((g) => g.status === 'COMPLETED'), competition: data.competition }" ></hockey-result>
      ...
      <hockey-fixture :data="{ ...data.games.find((g) => g.status === 'UPCOMING'), competition: data.competition }" />

      <hockey-fixture v-for="game in data.games.filter((g) => g.status === 'UPCOMING')" :data="{ ...game, competition: data.competition }" />
...
let data = ref(null);
axios.get(`http://localhost/api/seasons/1`)
.then((response) => {
