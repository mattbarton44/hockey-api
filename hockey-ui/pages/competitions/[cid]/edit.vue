<template>
  <div class="w-full">
    <div class="border-b border-gray-200 pb-5 sm:flex sm:items-center sm:justify-between">
      <h3 class="text-lg font-semibold leading-6 text-gray-900 py-2">Edit Competition</h3>
    </div>
...
api.competitions.show(cid, (response) => data.value = response);

const updateModel = () => {
  api.competitions.update(cid, data.value, () => navigateTo('/competitions/'));
}

const onFilePicked = (event) => {
  const files = event.target.files;
  data.value.logo = files[0];
  data.value.logoUrl = null;
}

definePageMeta({ requiresAuth: true, name: 'Edit Competition' })
</script>