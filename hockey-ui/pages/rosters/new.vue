<template>
  <div class="max-w-xl space-y-4">
    ...
    <propRow label="Team ID" :value="route.params.teamId" />
    <stackedInput label="Season">
      <select v-model="data.season_id" class="w-full border py-1.5 px-2 placeholder:text-gray-400 text-sm text-gray-900" >
        <option v-for="season in seasonsData" :value="season.id">{{ season.competition.name }} - {{ season.startYear }}/{{ season.endYear }}</option>
      </select>
    </stackedInput>
...
const createModel = () => {
  ...
  .then((response) => {
    ...
    navigateTo('/teams/'+ route.params.teamId)
  })
  .catch((e) => console.log(e))
}
