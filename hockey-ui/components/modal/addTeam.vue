          ...
          <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
            <DialogPanel class="relative transform overflow-hidden rounded-lg bg-white p-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
              <div class="sm:flex sm:items-start">
                <div class="">
                  <DialogTitle as="h3" class="text-base font-semibold leading-6 text-gray-900">Add Team</DialogTitle>
                  <div class="mt-4 space-y-4">
                    <div class="">
                      <label class="block text-sm font-medium leading-6 text-gray-900">New Team</label>
                      <div class="mt-2">
                        <select v-model="spawnData.team" class="py-1.5 px-2 flex w-full rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-great-blue-600 sm:max-w-md outline-none">
                          <option v-for="team in teams.filter((t) => data.rosters.filter((r) => r.team.id === t.id).length === 0)" :value="team">
                            {{ team.name }}
                          </option>
                        </select>
                      </div>
              ...
              <div class="flex mt-4">
                <button-action @click="addTeam" :disabled="spawnData.team.id === null">Add Team</button-action>
...
const teams = ref([]);
const spawnData = ref({ team: {} })

api.teams.index((d) => teams.value = d);

const addTeam = () => {
  if(spawnData.value.team.id !== null) {
    api.rosters.store({ season_id: sid, team_id: spawnData.value.team.id, players: [] }, (response) => emit('closeModal'));
  }
}
